...
function exportSVG() {
    ...
    // Add margin (0.5 inches) around the content
    ...
    // Convert to inches
    const plateLeft = minX / state.pixelsPerInch;
    const plateTop = minY / state.pixelsPerInch;
    const plateWidth = (maxX - minX) / state.pixelsPerInch;
    const plateHeight = (maxY - minY) / state.pixelsPerInch;

    // Start SVG with proper dimensions
    let svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg width="${plateWidth}in" height="${plateHeight}in" viewBox="0 0 ${plateWidth} ${plateHeight}" xmlns="http://www.w3.org/2000/svg">
    <desc>Guitar Harness Plate - Generated by VL Guitar Repair | ${state.holes.length} holes</desc>
    <defs>
        <style>
            .plate-outline { fill: #e0e0e0; stroke: #000; stroke-width: 0.01; }
            .hole-cutout { fill: #fff; stroke: #000; stroke-width: 0.005; }
            .trace-cutout { fill: none; stroke: #f00; stroke-width: 0.01; }
        </style>
    </defs>

    <!-- Plate Rectangle Frame -->
    <rect x="0" y="0" width="${plateWidth}" height="${plateHeight}" class="plate-outline"/>

    <!-- Hole Cutouts -->
    <g id="holes">
`;

    // Add each hole as a cutout circle
    state.holes.forEach((hole) => {
        const cx = (hole.x - minX) / state.pixelsPerInch;
        const cy = (hole.y - minY) / state.pixelsPerInch;
        const r = hole.diameter / 2;

        svg += `        <circle cx="${cx.toFixed(4)}" cy="${cy.toFixed(4)}" r="${r.toFixed(4)}" class="hole-cutout"/>\n`;
    });
    ...
    <g id="cutout">
        ...
        state.tracePoints.forEach((point, i) => {
            const x = ((point.x - minX) / state.pixelsPerInch).toFixed(4);
            const y = ((point.y - minY) / state.pixelsPerInch).toFixed(4);
            svg += i === 0 ? `${x},${y} ` : `L ${x},${y} `;
        });
